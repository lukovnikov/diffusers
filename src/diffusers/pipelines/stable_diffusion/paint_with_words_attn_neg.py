...
def inj_forward(self, hidden_states, encoder_hidden_states=None, attention_mask=None, **cross_attn_kwargs):
    ...
    if context is not None:
        if is_dict_format:
            ...
            cross_attention_weight = f(w, progress, attention_scores)
        else:
...
def paint_with_words(
    ...
    for t in tqdm(timesteps):
        ...
        progress = 1 - t / scheduler.num_train_timesteps
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                progress = 1 - t / self.scheduler.num_train_timesteps
...
def default_attn_neg_weight_fn(w, progress, qk, threshold=0.4, mult=1.):
    ...
    if progress < a:
        weight = 1
    elif b is not None and progress >= a and progress < b:
        midpoint = (b - a) / 2 \
                   + a
        weight = (math.sin(math.pi * (progress - (midpoint)) / (b - a)) + 1) * 0.5
        weight = 1 - weight
    else:
        weight = 0
