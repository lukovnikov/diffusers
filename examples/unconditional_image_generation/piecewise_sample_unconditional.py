...
from diffusers.pipelines.ddpm.pipeline_ddpm import PiecewiseDDPMPipeline
...
def main(args):
    ...
    # read phase specs:
    with open(os.path.join(args.loadmodel, "phase_setup.json"), "r") as f:
        phase_specs = json.load(f)
        phase_specs = {int(i): v for i, v in phase_specs.items()}

    print(json.dumps(phase_specs, indent=3))
    ...
    while cnt < args.numsamples:
        image = None
        # iterate over saved pipelines for each phase, generate image and feed it to next phase
        for phase in sorted(phase_specs.keys()):
            # if args.sampler == "ddim":
            #     ddimsched = _ddim_scheduler_from_ddpm_scheduler(pipeline.scheduler)
            #     pipeline = DDIMPipeline(pipeline.unet, ddimsched)

            pipeline = DiffusionPipeline.from_pretrained(os.path.join(args.loadmodel, f"phase{phase}"))
            ...
            num_steps = args.numsteps if args.numsteps != None else pipeline.scheduler.num_train_timesteps
            ...
            image = (torch.from_numpy(images).permute(0, 3, 1, 2) - 0.5) * 2
