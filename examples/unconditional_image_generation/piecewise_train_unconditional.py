...
def parse_args():
    ...
    parser.add_argument("--num_phases", type=int, default=16, help="How many phases during training (results in same number of models).")
...
def main(args):
    ...
    num_epochs_per_phase = int(math.ceil(args.num_epochs / args.num_phases))
    print(f"Real number of epochs: {num_epochs_per_phase * args.num_phases}")

    num_steps_per_phase = args.ddpm_num_steps // args.num_phases
    assert num_steps_per_phase == args.ddpm_num_steps / args.num_phases

    # save which timesteps are done in which phase
    phase_setup = {i: {} for i in range(args.num_phases)}
    for phase in range(args.num_phases):
        min_timestep = args.ddpm_num_steps - num_steps_per_phase * (phase + 1)
        max_timestep = args.ddpm_num_steps - num_steps_per_phase * phase
        phase_setup[phase] = (max_timestep, min_timestep)

    with open(os.path.join(args.output_dir, "phase_setup.json"), "w") as f:
        json.dump(phase_setup, f)
    ...
    for phase in range(args.num_phases):
        ...
        max_timestep, min_timestep = phase_setup[phase]
        ...
        for epoch in range(num_epochs_per_phase):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                # only sample timesteps for the current phase
                ...
                with accelerator.accumulate(model):
                    ...
                    if args.predict_epsilon:
                        ...
                        snr_weights = snr_weights / snr_weights.max()
                        if phase == args.num_phases - 1:
                            snr_weights.fill_(1.)       # equal weights if last phase because otherwise extreme values
        ...
        # save model at the end of every phase
        if accelerator.is_main_process:
            ...
            # save the model
            pipeline.save_pretrained(args.output_dir + f"/phase{phase}")
        accelerator.wait_for_everyone()
