...
class TextualInversionDataset(Dataset):
    ...
    def collate_fn(self, examples:List):
        ...
        tokout = self.tokenizer(texts, padding="longest", return_tensors="pt")
        batch = {}
        batch["input_ids"] = tokout.input_ids
        batch["attention_mask"] = tokout.attention_mask

        position_ids = StableDiffusionPipelineTIPP.generate_position_ids(batch["input_ids"], self.nonincids)
        batch["position_ids"] = position_ids

        if position_ids.max() >= self.tokenizer.model_max_length:
            raise Exception(f"Input is too long. Must be less than {self.tokenizer.model_max_length} tokens.")
...
def main():
    ...
    extratokenids = [tokenizervocab[extratoken] for extratoken in extratokens]
    nonincids = torch.tensor(extratokenids[1:])
    ...
    train_dataset.nonincids = nonincids
...
def tst_new_encode_prompt(prompt="hello this is a prompt", negprompt=None):
    pipe = StableDiffusionPipelineTIPP.from_pretrained("runwayml/stable-diffusion-v1-5")
    ret = pipe._encode_prompt(prompt, pipe.device, 2, True, negative_prompt=negprompt)
    print(ret)
