...
def _encode_text_color_inputs(
    ...
    """
    Tokenizes input text (input_prompt), encodes it using text encoder
    Args:
        text_encoder:
        tokenizer:
        device:
...
def paint_with_words(
    ...
    extra_seeds, seperated_word_contexts, encoder_hidden_states, uncond_encoder_hidden_states = \
        _encode_text_color_inputs(text_encoder, tokenizer, device, color_map_image, color_context, input_prompt,
                                  unconditional_input_prompt)

    # extra_seeds: ??
    # separated_word_contexts: ??
    # encoder_hidden_states: a dict that contains keys "CONTEXT_TENSOR", "CROSS_ATTENTION_WEIGHT_ORIG" and "CROSS_ATTENTION_WEIGHT_4096" etc for the mask at different resolutions
...
if __name__ == '__main__':
    ...
    settings = {
        ...
        "color_map_img_path": "images/test_image.png",
    ...
    img = paint_with_words(
        ...
        device="cuda:0",
        seed=420,
        weight_function=lambda w, sigma, qk: .5 * w * math.log(1 + sigma**2) * qk.std(),
    )
    #"""
